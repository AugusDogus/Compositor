...
impl Element {
    ...
    /// Make this element the group whose hover, presses and visible focus its descendants'
    /// [`Self::group_hover`], [`Self::group_active`] and [`Self::group_focus`] styles follow.
    ///
    ...
    /// Paint-only styling while the nearest [`Self::group`] ancestor owns visible focus.
    /// Uses the same input-modality rules as [`Self::focus`]. This lets a compound control
    /// paint its focus indicator on a checkbox or slider thumb without another tab stop.
    pub fn group_focus(self, style: impl FnOnce(ElementStateStyle) -> ElementStateStyle) -> Self {
        self.group_style(GroupState::Focus, None, style)
    }
    ...
    pub fn text_input_padding(mut self, padding: f32) -> Self {
        assert!(
            ...
            "text input padding must be finite and nonnegative"
        );
        let ElementKind::TextInput(input) = &mut self.kind else {
            panic!("text_input_padding can only be applied to a text input or text area");
        };
        input.horizontal_padding = padding;
        self
    }
    ...
    /// Disable this element and all currently attached descendants.
    /// Call after building the subtree. Unlike `disabled`, this also disables
    /// child controls, while preserving each control's own disabled styling.
    pub fn disable_subtree(&mut self) {
        fn disable(element: &mut Element) {
            element.accessibility.disabled = true;
