// The finite localization table below is adapted from Zed GPUI's macOS keyboard mapper,
// Copyright 2022-2025 Zed Industries, Inc., licensed under Apache-2.0. GPUI generated it
// by observing AppKit's automatic key-equivalent localization across Apple layouts. Keeping the
// data static avoids native probing, heap allocation, or input-event work in QuickGUI.

pub(crate) fn for_layout(layout_id: &str) -> &'static [(char, char)] {
    match layout_id {
...
mod tests {
    ...
    #[test]
    fn german_brackets_follow_apples_reachable_equivalents() {
        let german = for_layout("com.apple.keylayout.German");
        assert!(german.contains(&('[', 'ö')));
        assert!(german.contains(&(']', 'ä')));
    }

    #[test]
    fn layouts_without_special_equivalents_share_the_empty_table() {
        assert!(for_layout("com.apple.keylayout.US").is_empty());
        assert!(for_layout("com.example.custom-layout").is_empty());
    }
}
