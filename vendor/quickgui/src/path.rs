...
mod antialias;
use antialias::{BoundaryNormals, StrokeAntialias};
...
impl Path {
    fn from_geometry(
        ...
        if stroke_width.is_some() {
            vertex_normals.resize(geometry.vertices.len(), BoundaryNormals::default());
            for triangle in geometry.indices.as_chunks::<3>().0 {
                for i in 0..3 {
                    let first = triangle[(i + 1) % 3];
                    let second = triangle[(i + 2) % 3];
                    let edge = (first.min(second), first.max(second));
                    if edge_counts.get(&edge) != Some(&1) {
                        continue;
                    }
                    let a = geometry.vertices[first as usize];
                    let b = geometry.vertices[second as usize];
                    let inside = geometry.vertices[triangle[i] as usize];
                    let mut normal = [b[1] - a[1], a[0] - b[0]];
                    if normal[0] * (inside[0] - a[0]) + normal[1] * (inside[1] - a[1]) > 0. {
                        normal = [-normal[0], -normal[1]];
                    }
                    vertex_normals[first as usize].add(normal);
                    vertex_normals[second as usize].add(normal);
                }
            }
        }
        ...
        let stroke = stroke_width.map(|width| {
            StrokeAntialias::new(
                width,
                geometry.indices,
                vertex_normals,
                geometry.stroke_normals,
                geometry.stroke_centerline,
            )
        });
        let bytes = positions
            .len()
            .saturating_mul(mem::size_of::<[f32; 2]>())
            .saturating_add(stroke.as_ref().map_or(0, StrokeAntialias::byte_len))
    ...
    pub fn byte_len(&self) -> usize {
        self.0.positions.len() * mem::size_of::<[f32; 2]>()
            + self.0.stroke.as_ref().map_or(0, StrokeAntialias::byte_len)
            + self.0.boundary_masks.len()
    }
    ...
    pub(crate) fn stroke(&self) -> Option<&StrokeAntialias> {
        self.0.stroke.as_ref()
    }

    pub(crate) fn is_stroke(&self) -> bool {
        self.0.stroke.is_some()
    }
}
...
impl PathBuilder {
    ...
    pub fn build(self) -> Result<Path, PathError> {
        ...
        Path::from_geometry(
            geometry,
            match self.style {
                PathStyle::Stroke(options) => Some(options.line_width),
...
impl FillGeometryBuilder for LimitedGeometry {
    fn add_fill_vertex(&mut self, vertex: FillVertex) -> Result<VertexId, GeometryBuilderError> {
        self.add_vertex(vertex.position().to_array(), [0.; 2], [0.; 2])
    }
}
...
impl LimitedGeometry {
    fn add_vertex(
        ...
        self.stroke_normals.push(normal);
        self.stroke_centerline.push(centerline);
        Ok(id)
    }
}
