...
fn keyboard_focus_schedules_tooltips_but_pointer_focus_does_not_reopen_them() {
    ...
    assert!(tree.next_tooltip_deadline().is_none());
    // Moving off a clicked control must not reopen its tooltip from focus.
    tree.update_tooltip_hover(None, Instant::now());
    assert!(tree.next_tooltip_deadline().is_none());
    assert!(tree.focus_as(id, Some(InputModality::Keyboard)));
    assert!(tree.next_tooltip_deadline().is_some());
}
