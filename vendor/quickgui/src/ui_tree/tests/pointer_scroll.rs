...
#[test]
fn internal_drag_cursor_prefers_the_accepted_target_then_source_and_ignores_hover() {
    let source = ElementId::new(901);
    let target = ElementId::new(902);
    let mut tree = UiTree::new();
    let mut renderer = TestTextLayout;
    let mut scene = Scene::new();
    tree.set_root(
        div().size(200.0, 40.0).flex_row().children([
            div()
                .id(source)
                .size(100.0, 40.0)
                .cursor_text()
                .dragging(|s| s.cursor_alias()),
            div()
                .id(target)
                .size(100.0, 40.0)
                .cursor_text()
                .hover(|s| s.cursor_crosshair())
                .drag_over(|s| s.cursor_copy()),
        ]),
        Size::new(200.0, 40.0),
        1.0,
        &mut renderer,
    ...
    .unwrap();
    tree.paint(&mut scene, &mut renderer).unwrap();
    assert_eq!(tree.internal_drag_cursor_style(), CursorStyle::ClosedHand);
    tree.begin_drag(source);
    assert_eq!(tree.internal_drag_cursor_style(), CursorStyle::DragLink);
    tree.set_drag_over(Some(target));
    assert_eq!(tree.internal_drag_cursor_style(), CursorStyle::DragCopy);
    tree.set_drag_over(None);
    assert_eq!(tree.internal_drag_cursor_style(), CursorStyle::DragLink);
    tree.end_drag();
    assert_eq!(tree.internal_drag_cursor_style(), CursorStyle::ClosedHand);
    tree.begin_drag(target);
    tree.set_drag_over(Some(source));
    assert_eq!(tree.internal_drag_cursor_style(), CursorStyle::ClosedHand);
}
