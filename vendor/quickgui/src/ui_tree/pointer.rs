...
impl UiTree {
    ...
    /// Cursor for a local drag, independent of unrelated controls under the pointer.
    pub(crate) fn internal_drag_cursor_style(&self) -> CursorStyle {
        let region = |id: Option<ElementId>| {
            let id = id?;
            self.hit_regions.iter().rev().find(|region| region.id == id)
        };
        region(self.drag_over)
            .and_then(|region| region.cursor_states.drag_over)
            .or_else(|| region(self.dragging).and_then(|region| region.cursor_states.dragging))
            .unwrap_or(CursorStyle::ClosedHand)
    }
