...
impl<'a> GroupScope<'a> {
    ...
    pub(super) fn for_children(
        ...
        element.group.then(|| GroupScope {
            ...
            focused: styled_focus == Some(element.runtime_id),
        })
    }
        ...
        while let Some(scope) = current {
            if target.is_none_or(|name| scope.name == Some(name)) {
                return match state {
                    ...
                    GroupState::Focus => scope.focused,
                };
            }
...
pub(super) fn collect_layout_hit_regions(
    ...
    *source_order = (*source_order).saturating_add(1);
    ...
    let own_scope = GroupScope::for_children(element, hovered, pressed_path, styled_focus, groups);
    ...
    *source_order = (*source_order).saturating_add(1);
    ...
    match &element.kind {
        ...
        ElementKind::TextInput(input) => {
            ...
            #[cfg(feature = "text-input-decorations")]
            let (right_inset, left_inset) = input.editor.map_or(
                (input.horizontal_padding, input.horizontal_padding),
                |editor| (editor.content_padding_right, editor.content_padding_left),
            );
            #[cfg(not(feature = "text-input-decorations"))]
            let (right_inset, left_inset) = (input.horizontal_padding, input.horizontal_padding);
