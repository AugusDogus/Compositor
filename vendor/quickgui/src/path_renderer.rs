    ...
    pub(crate) fn prepare(
        ...
        for layer in scene.paint_layers() {
            ...
            for item in layer.paint() {
                ...
                let vertex_count = antialiased_vertex_count(&primitive.path);
                if !admit_path(
                    &mut admitted_paths,
            ...
            for pending in &self.pending {
                ...
                append_path_vertices(&mut self.vertices, primitive, paint_index, scale);
...
fn visible_path(primitive: &PathPrimitive, viewport: Rect) -> bool {
    primitive
        ...
        .unwrap_or(viewport)
        .intersection(viewport)
        .is_some_and(|clip| primitive.antialias_bounds().intersects(clip))
}

fn antialiased_vertex_count(path: &crate::Path) -> usize {
    let fringe_vertices = if path.is_stroke() {
        path.boundary_masks()
            .iter()
            .map(|mask| mask.count_ones() as usize * 6)
            .sum()
    } else {
        0
    };
    path.vertex_count() + fringe_vertices
}
...
fn append_path_vertices(
    ...
    const BARYCENTRICS: [[f32; 3]; 3] = [[1., 0., 0.], [0., 1., 0.], [0., 0., 1.]];
    for (triangle_index, (triangle, boundary_mask)) in primitive
        .path
        .positions()
        .as_chunks::<3>()
        .0
        ...
        .enumerate()
    {
        if let Some(stroke) = primitive.path.stroke() {
            let samples: [(GpuPathVertex, GpuPathVertex); 3] = std::array::from_fn(|vertex| {
                let index = triangle_index * 3 + vertex;
                let offset = stroke.offset(index, primitive.scale_factors(), display_scale);
                let width = stroke.width_pixels(index, primitive.scale_factors(), display_scale);
                let opacity = (width / display_scale.min(1.)).clamp(0., 1.);
                let inner = 0.5 * opacity;
                let outer = 1. - inner;
                let sample = |amount: f32, coverage| GpuPathVertex {
                    position: [
                        triangle[vertex][0] + offset[0] * amount,
                        triangle[vertex][1] + offset[1] * amount,
                    ],
                    barycentric: [coverage, 0., 0.],
                    // A negative mask selects interpolated fringe coverage. Filled
                    // paths retain their existing triangle-boundary coverage.
                    edge_mask: [-1., 0., 0.],
                    paint_index,
                };
                let mut core = sample(-inner, opacity);
                if opacity < 1. {
                    // Collapse thin cores onto Lyon's exact centerline. An
                    // inset reconstructed from boundary miters can invert a
                    // narrow corner and blend the same stroke more than once.
                    core.position = stroke.centerline(index);
                }
                (core, sample(outer, 0.))
            });
            vertices.extend(samples.iter().map(|(inner, _)| *inner));
            for edge in 0..3 {
                if boundary_mask & (1 << edge) != 0 {
                    let (first, second) = ((edge + 1) % 3, (edge + 2) % 3);
                    let (a, outer_a) = samples[first];
                    let (b, outer_b) = samples[second];
                    vertices.extend([a, b, outer_b, a, outer_b, outer_a]);
                }
            }
        } else {
            let edge_mask = [
...
mod tests {
    ...
    fn expands_boundary_masks_and_paint_indices() {
        ...
        append_path_vertices(&mut vertices, &primitive, 7, 1.);
    ...
    #[test]
    fn stroke_admission_reserves_fringe_vertices_before_upload() {
        let mut builder = PathBuilder::stroke(2.);
        builder.move_to(Point::new(0., 0.));
        builder.line_to(Point::new(100., 0.));
        let path = builder.build().unwrap();
        let primitive = PathPrimitive::new(&path, Color::WHITE);
        let mut emitted = Vec::new();
        append_path_vertices(&mut emitted, &primitive, 0, 1.);
        let count = antialiased_vertex_count(&path);
        assert_eq!(count, emitted.len());
        assert!(count > path.vertex_count());

        // The tessellated core fits, but its coverage fringe does not.
        let mut paths = 1;
        let mut vertices = MAX_GPU_PATH_VERTICES - path.vertex_count();
        assert!(!admit_path(
            &mut paths,
            &mut vertices,
            count,
        ...
        assert_eq!(paths, 1);
        assert_eq!(vertices, MAX_GPU_PATH_VERTICES - path.vertex_count());
        // Rejection must not consume the space remaining for a smaller fill.
        assert!(admit_path(
            &mut paths,
            &mut vertices,
