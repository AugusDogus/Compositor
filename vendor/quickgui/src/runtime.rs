...
struct RuntimeWindow {
    ...
    cursor: CursorIcon,
    cursor_override: Option<(crate::CursorOverrideId, winit::window::Cursor)>,
...
struct Runtime {
    ...
    #[cfg(target_os = "linux")]
    wayland_activation: Option<wayland_activation::Requests>,
...
fn runtime_window_state(
    ...
    WindowState {
        ...
        cursor_override: state.cursor_override.as_ref().map(|(id, _)| *id),
...
fn desired_cursor(state: &RuntimeWindow, point: Point) -> CursorIcon {
    ...
    if state.drag_session.is_some() {
        return platform_cursor(state.ui.internal_drag_cursor_style());
    }
...
fn cursor_alpha(window: &Window) -> crate::cursor_image::NativeCursorAlpha {
    use crate::cursor_image::NativeCursorAlpha;
    #[cfg(not(target_os = "macos"))]
    if matches!(
        window.window_handle().map(|handle| handle.as_raw()),
        Ok(winit::raw_window_handle::RawWindowHandle::Xlib(_)
            | winit::raw_window_handle::RawWindowHandle::Xcb(_))
    ) {
        return NativeCursorAlpha::Premultiplied;
    }
    let _ = window;
    NativeCursorAlpha::Straight
}

fn set_cursor_if_changed(state: &mut RuntimeWindow, cursor: CursorIcon) {
    if cursor != state.cursor {
        state.cursor = cursor;
        if state.cursor_override.is_none() {
