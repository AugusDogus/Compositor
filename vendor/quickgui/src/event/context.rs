...
impl EventContext {
    ...
    /// Start a native move from a mouse-down listener in a custom titlebar.
    pub fn begin_window_move(&mut self) -> Result<(), WindowCommandError> {
        let handle = self.current_window_handle()?;
        self.push_window_command(WindowCommand::BeginMove(handle))
    }

    /// Start a native resize from a mouse-down listener on a custom window edge.
    pub fn begin_window_resize(
        &mut self,
        edge: crate::ResizeDirection,
    ) -> Result<(), WindowCommandError> {
        let handle = self.current_window_handle()?;
        self.push_window_command(WindowCommand::BeginResize(handle, edge))
    }
    ...
    /// Override this window's element cursors with a native shape or bitmap.
    /// Pass `None` to resume the cursor selected by hit testing. Visibility remains independent.
    pub fn set_cursor_override(
        &mut self,
        image: Option<crate::CursorOverride>,
    ) -> Result<(), WindowCommandError> {
        let handle = self.current_window_handle()?;
        self.push_window_command(WindowCommand::SetCursorOverride(handle, image))
    }
