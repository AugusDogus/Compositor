...
impl AppRunner {
    /// Retain the native display for integrations that share its connection.
    pub fn owned_display_handle(&self) -> winit::event_loop::OwnedDisplayHandle {
        self.event_loop.owned_display_handle()
    }

    /// Use an application-owned provider for the general clipboard, including
    /// native text controls. Install it before creating application windows.
    pub fn set_clipboard_provider(&mut self, provider: impl crate::ClipboardProvider + 'static) {
        self.runtime.clipboard.set_general_provider(provider);
    }
