...
impl Editor {
    pub(in crate::ui) fn camera_raw_controls(&self, cx: &mut ViewContext<'_, Self>) -> Element {
        let mut groups = div().flex_row().flex_wrap().gap(4.);
        for group in Group::ALL {
            groups = groups.child(
                Self::check_control(group.name(), self.camera_raw.group == group).on_click(
                    cx.listener(
                        format!("camera-group-{}", group as usize),
                        move |this, cx| {
                            this.camera_change(|edit| edit.group = group);
                            this.changed(cx);
                        },
                    ),
                ),
        ...
        match group {
            ...
            Group::Effects => {
                ...
                for (label, style) in [
                    ...
                    choices = choices.child(
                        Self::check_control(
                            label,
                            self.camera_raw.settings.vignette_style == style,
                        )
                        .on_click(cx.listener(
                            format!("camera-vignette-{label}"),
                            move |this, cx| {
            ...
            Group::Curve => {
                for (i, label) in ["RGB", "Red", "Green", "Blue"].into_iter().enumerate() {
                    choices = choices.child(
                        Self::check_control(label, self.camera_raw.channel == i).on_click(
                            cx.listener(format!("camera-curve-{i}"), move |this, cx| {
                                this.camera_change(|e| e.channel = i);
                                this.changed(cx);
                            }),
                        ),
                    );
            ...
            Group::Mixer => {
                ...
                if self.camera_raw.mixer_page == MixerPage::Families {
                    for (i, label) in ColorMixer::NAMES.into_iter().enumerate() {
                        choices = choices.child(
                            Self::check_control(label, self.camera_raw.color == i).on_click(
                                cx.listener(format!("camera-color-{i}"), move |this, cx| {
                    ...
                    if !self.camera_raw.settings.mixer.points.is_empty() {
                        choices = choices.child(Self::control("Remove color").on_click(
                            cx.listener("camera-point-remove", |this, cx| {
                                this.camera_change(|e| {
            ...
            Group::Grading => {
                ...
                {
                    choices = choices.child(
                        Self::check_control(label, self.camera_raw.wheel == i).on_click(
                            cx.listener(format!("camera-grade-{i}"), move |this, cx| {
                                this.camera_change(|e| e.wheel = i);
                                this.changed(cx);
                            }),
                        ),
                    );
                    ...
                    .child(
                        Self::check_control("Lens profile", self.camera_raw.settings.lens_profile)
                            .on_click(cx.listener("camera-lens-profile", |this, cx| {
                                this.camera_change(|e| {
                                    e.settings.lens_profile = !e.settings.lens_profile
                                });
                                this.changed(cx);
                            })),
                    );
            ...
            Group::Geometry => {
                for (label, projection) in [
                    ...
                    choices = choices.child(
                        Self::check_control(
                            label,
                            self.camera_raw.settings.projection == projection,
                        )
                        .on_click(cx.listener(
                ...
                choices = choices
                    ...
                    .child(
                        Self::check_control(
                            "Constrain crop",
                            self.camera_raw.settings.constrain_crop,
                        )
                        .on_click(cx.listener(
                            "camera-constrain",
                            |this, cx| {
    ...
    fn camera_auto_balance(&mut self) {
        let Ok((document, _)) = self.filter_source() else {
            return;
        };
        let Some(source) = document.active_layer().and_then(|l| l.raster()) else {
            return;
        };
        match compositor::camera_raw::auto_balance(source) {
            Ok([temperature, tint]) => self.camera_change(|e| {
