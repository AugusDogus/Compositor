...
pub(super) fn sample(image: &Rgb32FImage, x: f32, y: f32) -> [f32; 3] {
...
pub fn render(
    ...
    if let Some(bytes) = accelerated(raw, settings, cancel, 8)? {
        let [l, t, r, b] = super::gpu::crop(settings, [raw.camera.width(), raw.camera.height()]);
        return RgbaImage::from_raw(r - l, b - t, bytes)
            .ok_or_else(|| invalid("RAW GPU returned the wrong number of pixels."));
    }
...
pub fn render_16(
    ...
    if let Some(bytes) = accelerated(raw, settings, cancel, 16)? {
        ...
        let pixels = bytes
            ...
            .collect();
        return ImageBuffer::from_raw(r - l, b - t, pixels)
            .ok_or_else(|| invalid("RAW GPU returned the wrong number of 16-bit pixels."));
    }
...
fn accelerated(
    ...
    let result = super::gpu::develop(raw, settings, wb, depth, cancel)?;
    cancelled(cancel)?;
    Ok(result)
}

pub(super) fn render_at_depth<T: Primitive + Send + Sync>(
    raw: &DecodedRaw,
    settings: &DevelopSettings,
    cancel: &AtomicBool,
    encode: fn(f32) -> T,
