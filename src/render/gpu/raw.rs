...
#[path = "raw_engine.rs"]
mod raw_engine;
use crate::raw::ensure;
use crate::raw::{DecodedRaw, DevelopSettings, Overlay, OverlayKind};
use crate::{Result, invalid};
use raw_engine::Processor;
use std::sync::atomic::{AtomicBool, Ordering};

const SHADER: &str = concat!(
    include_str!("raw_buffers.wgsl"),
    include_str!("raw_raster.wgsl"),
...
pub(crate) fn develop(
    ...
    if u64::from(raw.camera.width()) * u64::from(raw.camera.height()) < 16_384 {
        return Ok(None);
    }
    let Ok(engine) = raw_engine::engine() else {
        return Ok(None);
    };
    let engine = engine.lock().map_err(|_| {
        invalid("RAW GPU worker stopped. Restart the editor; the camera file is unchanged.")
    })?;
    if !fits_hardware(
        [raw.camera.width(), raw.camera.height()],
        &engine.device.limits(),
    ) {
        eprintln!(
            "RAW {}×{} exceeds this GPU's storage or dispatch limits. Using native CPU development.",
            raw.camera.width(),
            raw.camera.height()
        );
        return Ok(None);
    ...
    let result = engine.develop(raw, s, wb, depth, cancel);
    if let Some(error) = errors.finish() {
        return Err(invalid(format!(
            "RAW GPU development failed: {error}. The camera file and project are unchanged."
        )));
    }
    result.map(Some)
}

/// Check storage and dispatch requirements before allocating full-resolution buffers.
pub(crate) fn fits_hardware(size: [u32; 2], limits: &wgpu::Limits) -> bool {
    let Some(bytes) = (u64::from(size[0]) * u64::from(size[1])).checked_mul(16) else {
        return false;
    };
    !size.contains(&0)
        ...
        && size
            .iter()
            .all(|dimension| dimension.div_ceil(8) <= limits.max_compute_workgroups_per_dimension)
}
